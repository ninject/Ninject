...
namespace Ninject.Tests.Unit.ActivationScopeTests
{
	...
	public class WhenResolveIsCalledWithServiceType : ActivationScopeContext
	{
		...
		{
			scope.Resolve(typeof(IWeapon), new Func<IBindingMetadata, bool>[0], new IParameter[0]);
			parentMock.Verify(x => x.Resolve(It.Is<Request>(r => r.Service == typeof(IWeapon) && r.Constraints.Count == 0 && r.Parameters.Count == 0)));
		}
